...
- Never add Redis or set `replicas` > 1 — deliberate post-v1 deferral, not an oversight: [redis-deferred](redis-deferred.md)
- Never delete dormant Epic 6 ticketing code — feature-flagged off, not rolled back: [ticketing-dormant](ticketing-dormant.md)
...
- `apps/strapi/src/plugins/entity-properties/` is an empty shell awaiting deletion — never build on it: [entity-properties-vestigial](entity-properties-vestigial.md)
- Never bump `prettier` past 3.3.2 — it corrupts Strapi's generated TS schemas

## Commands

- Yarn 1 classic only — never npm/pnpm
- `yarn test` runs **Vitest in `apps/client`, Jest in `apps/strapi`**: [test-layout](test-layout.md)
...
- In a git worktree, run `yarn install --frozen-lockfile` before any test, lint, or build: [worktree-install](worktree-install.md)
- Never report a story verified when its verification command could not execute — that story is `blocked`, not `done`

## Process

- Ship from `main` directly: no PRs, no feature branches: [commit-and-push-policy](commit-and-push-policy.md)
...
- Never let a generator write `_bmad-output/implementation-artifacts/sprint-status.yaml`: [sprint-status-hand-maintained](sprint-status-hand-maintained.md)
...
- New backend features are plugins under `src/plugins/`, never `src/api/`: [strapi compass](compass/strapi.md)
- Cross-plugin calls go through the target's `services/public-api.ts` facade only — never `strapi.documents()` with a foreign UID: [plugin-dependency-rules](plugin-dependency-rules.md)
...
- Strapi v5 responses are flat — `response.data.title`. There is no `attributes` wrapper; never remap one: [strapi-v5-shape](strapi-v5-shape.md)
...
- Arabic uses Western digits and `DD/MM/YYYY` — never format with a raw `ar` locale: [arabic-western-numerals](arabic-western-numerals.md)
