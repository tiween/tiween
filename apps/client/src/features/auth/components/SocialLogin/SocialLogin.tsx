"use client"

import * as React from "react"
import { Loader2 } from "lucide-react"

import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"

/**
 * Localized labels for SocialLogin
 */
export interface SocialLoginLabels {
  google: string
  facebook: string
  divider: string
}

const defaultLabels: SocialLoginLabels = {
  google: "Continuer avec Google",
  facebook: "Continuer avec Facebook",
  divider: "ou",
}

export interface SocialLoginProps {
  /** Called when Google button is clicked */
  onGoogleClick: () => void
  /** Called when Facebook button is clicked */
  onFacebookClick: () => void
  /** Show loading spinner on Google button */
  isGoogleLoading?: boolean
  /** Show loading spinner on Facebook button */
  isFacebookLoading?: boolean
  /** Disable all buttons */
  disabled?: boolean
  /** Localized labels */
  labels?: SocialLoginLabels
  /** Additional class names */
  className?: string
}

/**
 * Google logo SVG icon
 */
function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  )
}

/**
 * Facebook logo SVG icon
 */
function FacebookIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"
        fill="#1877F2"
      />
    </svg>
  )
}

/**
 * SocialLogin - OAuth provider buttons for Google and Facebook
 *
 * Features:
 * - Branded buttons with official icons
 * - Loading states per provider
 * - Divider with "ou" text
 * - RTL support via CSS logical properties
 *
 * @example
 * ```tsx
 * <SocialLogin
 *   onGoogleClick={() => signIn("google")}
 *   onFacebookClick={() => signIn("facebook")}
 *   isGoogleLoading={isLoading && provider === "google"}
 * />
 * ```
 */
export function SocialLogin({
  onGoogleClick,
  onFacebookClick,
  isGoogleLoading = false,
  isFacebookLoading = false,
  disabled = false,
  labels = defaultLabels,
  className,
}: SocialLoginProps) {
  const isAnyLoading = isGoogleLoading || isFacebookLoading

  return (
    <div className={cn("flex flex-col gap-3", className)}>
      {/* Google button */}
      <Button
        type="button"
        variant="outline"
        size="lg"
        onClick={onGoogleClick}
        disabled={disabled || isAnyLoading}
        className="relative w-full"
      >
        {isGoogleLoading ? (
          <Loader2 className="me-2 h-5 w-5 animate-spin" />
        ) : (
          <GoogleIcon className="me-2 h-5 w-5" />
        )}
        {labels.google}
      </Button>

      {/* Facebook button */}
      <Button
        type="button"
        variant="outline"
        size="lg"
        onClick={onFacebookClick}
        disabled={disabled || isAnyLoading}
        className="relative w-full"
      >
        {isFacebookLoading ? (
          <Loader2 className="me-2 h-5 w-5 animate-spin" />
        ) : (
          <FacebookIcon className="me-2 h-5 w-5" />
        )}
        {labels.facebook}
      </Button>

      {/* Divider */}
      <div className="relative my-2">
        <div className="absolute inset-0 flex items-center">
          <span className="border-border w-full border-t" />
        </div>
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-background text-muted-foreground px-2">
            {labels.divider}
          </span>
        </div>
      </div>
    </div>
  )
}

SocialLogin.displayName = "SocialLogin"
