...
- source_spec: `_bmad-output/implementation-artifacts/2b-16-events-manager-plugin-test-coverage.md`
  ...
  severity: high
  summary: The backend suites this story exists to provide run in no automated gate, and the ≥80% coverage gate is neither measured nor enforced.
  evidence: AMENDED 2026-08-11 (`spec-2b-16-supertest-controller-tests.md`) — two claims below were wrong or overstated and are corrected in place. STILL TRUE (the substance): `test:integration` is invoked by NO workflow (`.github/workflows/ci.yml` runs bare `yarn test`), so the boot-based suites — now seven, including the rewritten controller suite — execute in no automated gate; and no `coverageThreshold` or `collectCoverage` exists anywhere (a `test:coverage` script exists but no workflow invokes it). Also still true: the 24 cases in `event-manager.unit.test.ts` that DO run are genuine, not tautological. NOW FALSE, do not re-report: the claim that "the controller AC was substituted … the suite builds its own Koa stub and asserts on jest mocks, so route wiring, admin auth and policies are untested" — `event-manager.controller.test.ts` is now 19 Supertest cases against `strapi.server.httpServer` authenticated with a `createAdminSession` token, no `ctx` constructed; route wiring is proven by mutation (deleting a route from `routes/index.ts` fails exactly the tests that exercise it) and the admin-auth boundary is asserted directly (401 unauthenticated on all four routes, plus a valid users-permissions JWT rejected). OVERSTATED: "silently skipped by `yarn test`" — the `testMatch: ["**/*.unit.test.ts"]` exclusion is a deliberate, documented design (`apps/strapi/jest.config.cjs:51-55`) that keeps a boot/env failure from blocking the unit gate; the gap is that nothing else then runs them, not that the exclusion is a mistake. Equally, "`(yarn build:test-dist || true)` swallows build failure" — the swallow is deliberate (pre-existing TS errors elsewhere must not skip the suite) AND already mitigated by `assertTestDistFresh` (`apps/strapi/tests/helpers/test-dist.ts:57`, called from `setupStrapi`), which refuses to boot on a stale `dist/` so a non-emitting build fails loudly instead of testing old code. ORIGINAL (2026-08-11 review, superseded): the server jest project's `testMatch` is `["**/*.unit.test.ts"]` only, so `event-manager.service.test.ts`, `event-manager.controller.test.ts` and `tests/app.test.js` are silently skipped by `yarn test` — the only test command CI runs; `test:integration` is invoked by no workflow and begins `(yarn build:test-dist || true)`, which swallows build failure; no `coverageThreshold` or `collectCoverage` exists anywhere; the controller AC was also substituted.
  status: open

- source_spec: `_bmad-output/implementation-artifacts/1-1-initialize-monorepo-from-starter-template.md`
  location: apps/client/package.json:8 (`dev`), apps/strapi/package.json (`develop`), .tool-versions:1
  severity: low
...
- source_spec: none
  ...
  evidence: Split from the 2B.16 closeout intent as the low-severity remainder. `tests/helpers/strapi.ts:95` uses `fs.existsSync` in teardown. (The companion claim in this entry — one `any`/eslint-disable surviving in `event-manager.controller.test.ts` — was overtaken on 2026-08-11: that file was rewritten for AC#3 and now contains neither, so only the `fs.existsSync` half remains open.) Verified 2026-08-11 that three sibling findings from the same 2026-06 review are already resolved and need no work: the legacy `tests/helpers/strapi.js` is deleted, the test DB path is now `path.join`-anchored, and fixture cleanup paginates at `limit: 100` rather than capping at 1000.

## Deferred from: review of spec-2b-16-supertest-controller-tests.md (2026-08-11)

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/src/plugins/events-manager/server/src/routes/index.ts:159-167
  severity: high
  summary: `POST /events-manager/seed` is annotated "admin only" but carries `auth: false`, so an unauthenticated caller can invoke `seedService.seedAll()` and write venues and event groups.
  evidence: Found independently by two review layers and confirmed in source. The route sits in the plugin's `admin` route group under the comment `// Seed routes (admin only)` yet sets `config.auth = false`; its sibling `POST /seed/event-groups` at :168 omits `auth` and is therefore correctly protected. That asymmetry, plus the contradicting comment, reads as an oversight rather than a decision. `seed.seed` (`server/src/controllers/seed.ts:4-17`) calls `seedService.seedAll()` with no caller check. Not caused by the AC#3 test work — pre-existing, surfaced incidentally because the new suite audits the admin-auth boundary of the four sibling routes in the same table. Fix: drop `auth: false`, or if an unauthenticated dev-seed hook is genuinely wanted, gate it on `NODE_ENV !== "production"` and say so in the comment.

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/src/plugins/events-manager/server/src/routes/index.ts:125-158
  severity: medium
  summary: The four events-manager admin routes register no RBAC permission action and carry `policies: []`, so any authenticated admin regardless of role can bulk-create screenings, duplicate events and rewrite ticket inventory.
  evidence: No `actionProvider.registerMany` call covers these routes and every entry declares an empty policy array, so Strapi's default admin authentication is the only guard — authentication, not authorization. The new Supertest suite proves "logged in or not" and deliberately stops there, since AC#3 is about transport. `tests/helpers/admin.ts:21-28` already supports `createAdminSession(strapi, { permissions: [...] })` for exactly this kind of scoped-role test, and `venue-admin-crud.service.test.ts:220-224` demonstrates the pattern, so the test-side cost is low once the permission actions exist. Fix: register admin permission actions for the four routes and add scoped-role coverage.

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/src/plugins/events-manager/server/src/controllers/event-manager.ts:116-118
  severity: low
  summary: `getEventStats`'s `if (!eventId) return ctx.badRequest(...)` guard is unreachable over HTTP and is now dead code.
  evidence: The only route reaching that handler is `GET /event-stats/:eventId`, whose path segment is required — a request without it does not match the route and is answered 404 by the router before the handler runs. The old ctx-stub suite covered the guard only because it invoked the handler directly with an empty params object, which is precisely the bypass AC#3 forbids. Surfaced, not caused, by the rewrite. Fix: delete the guard, or keep it and mark it defensive with a comment so a future reader does not mistake it for reachable validation.

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/src/plugins/events-manager/server/src/controllers/event-manager.ts:9-19,56
  severity: medium
  summary: The controller's body-forwarding of `videoFormat`, `audioLanguage`, `subtitleLanguage`, `price`, `dateOffset` and `copySubEvents` is exercised by no test at any layer.
  evidence: `createBulkScreenings` destructures and forwards four optional fields and `duplicateEvent` forwards two; no request body in either the new Supertest suite or the surviving unit suite sends any of them. A controller that silently dropped `price` or `copySubEvents` from its destructure would leave every test green — exactly the defect class an HTTP-level controller suite exists to catch. `copySubEvents: true` is the most valuable gap, since it drives a whole service branch that copies screenings and performances. Not a regression: the previous ctx-stub suite did not cover them either, and the AC#3 rewrite preserved coverage at parity by design.

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/tests/fixtures/events.ts:144-154
  severity: medium
  summary: The `performance` sub-event kind is untested, and `cleanupContent` does not delete performance rows, so adding that coverage later will leak state between tests.
  evidence: `updateTicketInventory` accepts `kind: "screening" | "performance"` and resolves a different UID per kind; every test sends `"screening"`. An unknown `kind` (service throws `Invalid kind`) is also uncovered. Compounding it, `cleanupContent` deletes screening / event / work / character / credit-role / venue but never `plugin::events-manager.performance` — so the first performance-kind test added will leave rows behind for every subsequent test in the run. Fix the fixture in the same change that adds the coverage, not after.

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/src/plugins/events-manager/server/src/routes/**tests**/venue-routes.unit.test.ts:22-47
  severity: medium
  summary: A cheap route-table guard in the CI-gated unit project would cover the four admin routes' auth declarations without waiting on the CI-integration story.
  evidence: Complements — does not duplicate — the existing high-severity entry above about `test:integration` running in no automated gate. The point here is that the gap has a cheap partial mitigation available today: `venue-routes.unit.test.ts` already pins the six content-api `/venue/*` route entries in a `*.unit.test.ts` file, which the `server` jest project DOES collect and CI DOES run. An equivalent guard asserting each of the four admin entries has the expected method/path, no `auth` key, and a handler present on the controller factory would catch an accidental `auth: false` (the exact shape of the `/seed` defect above) inside the existing gate, with no boot and no DB. The Supertest suite remains the stronger check; this is the part that can run everywhere.

- source_spec: `_bmad-output/implementation-artifacts/spec-2b-16-supertest-controller-tests.md`
  location: apps/strapi/src/plugins/events-manager/server/src/services/event-manager.ts (duplicateEvent slug generation)
  severity: low
