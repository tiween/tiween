...
- [x] [Review][Patch] **(resolved before 2026-08-10 by later work — verified, no change needed)** `yarn seed` is broken — `seedEntityProperties` calls the deleted entity-properties seed service [apps/strapi/scripts/seeds/index.ts:542] — `strapi.plugin("entity-properties").service("seed")` now resolves to `{}` (emptied shell); `.seedAll("en")` throws `TypeError`. Called at line 578 BEFORE all other seeds, so the entire `yarn seed` pipeline dies (catch does `process.exit(1)`). Corollary: property categories/definitions are never seeded by any caller — the relocated `seedPropertyCategories`/`seedPropertyDefinitions` in the venues plugin have no production invoker. Fix: repoint to `strapi.plugin("venues").service("seed").seedPropertyDefinitions("en")`.
- [x] [Review][Patch] **(fixed 2026-08-10)** Controllers violate guardrail R7 — untyped `ctx` + prose error message [apps/strapi/src/plugins/venues/server/src/controllers/index.ts:9,33] — `findVenues(ctx)`/`findVenue(ctx)`/`seedVenues(ctx)` lack `: Context` typing; `findVenue` returns `ctx.notFound("Venue not found")` (prose) instead of a CODE like `ctx.notFound("VENUE_NOT_FOUND")`. New code, in scope. R7 mandates typed ctx + error codes.
- [x] [Review][Patch] **(fixed 2026-08-10)** Stale UID in docs defeats the AC10 grep gate [apps/strapi/docs/plugin-architecture.md:81] — still reads `plugin::events-manager.venue`. AC10 requires zero `events-manager.venue` references repo-wide; the dev grep gate excluded docs.
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-10 | Review closure (gate for Story 2D.2). Finding 1 (`yarn seed` broken) verified already resolved by later work — `seedEntityProperties` now delegates to `venues.seed.seedPropertyDefinitions`. Finding 2 fixed: `findVenues`/`findVenue`/`seedVenues` typed `ctx: Context` (koa); prose errors → CODES (`VENUE_NOT_FOUND`, `VENUE_SEED_FAILED`). Finding 3 fixed: `docs/plugin-architecture.md` venue row moved out of events-manager into a new venues-plugin section. Grep gate re-run clean (only i18n translation keys remain, not UIDs); `yarn type-check` clean; strapi suite 76/76 suites, 1132/1132 tests green. Status → done. |
