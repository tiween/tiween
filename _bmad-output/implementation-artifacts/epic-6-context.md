...
- Payment methods: e-Dinar, Sobflous, D17, Flouci, Carte bancaire. Payment must complete within 5 seconds (NFR-IN1). Failed payments show a clear error with a retry path, and the user is never charged on failure.
...
- Confirmation email arrives within 2 minutes (NFR-IN3) and includes order details, event info, one QR per ticket, and an add-to-calendar link; it goes to the account email or the guest email.
...
- UI copy is French: "Mes Billets", "Mes achats", "Paiement validé!", "Complet", "Historique".
...
- **Ticketing Unit of Work (prerequisite, Epic 2C.4 / arch step 3):** `order.createOrder` runs inside `strapi.db.transaction` — re-read availability → create order + tickets → decrement inventory via the events-manager facade — throwing the `TICKET_SOLD_OUT` code on failure to roll back. Cross-plugin facade calls that write happen inside the caller's transaction; facades never open their own.
- **Atomic inventory (concurrency floor):** `events-manager.public-api.adjustInventory(subEventId, kind, delta)` performs a single conditional `UPDATE ... WHERE tickets_sold + qty <= tickets_available`; zero rows affected throws `TICKET_SOLD_OUT`. This is the one sanctioned exception to the Document-Service-only rule.
...
- **Real-time availability (6.10):** WebSocket-based live "X remaining" updates with graceful degradation on disconnect. This falls under the deferred real-time FR cluster (FR64–66) built on top of the inventory facade; the original architecture's WebSocket decision governs it.
...
- Domain components: `ShowtimePicker`, `QuantitySelector`, `TicketQR`/`TicketCard`, and shadcn-based `CheckoutForm`; the app shell uses a persistent bottom tab bar (Accueil / Recherche / Billets / Compte).
- Confirmation moment is a deliberate celebration: confetti + yellow checkmark animation, "Paiement validé!" message, order summary, a prominent "View my tickets" CTA, add-to-calendar, and share.
...
- **Hard prerequisite:** Epic 2C.4 (ticketing Unit of Work + atomic inventory facade) must be complete before stories 6.1 and 6.3 begin; story 6.10 builds directly on the `events-manager.public-api` inventory facade.
- The `payments` plugin is created as the first infrastructure task when the epic starts (arch migration "With Epic 6" checklist).
