  ...
  return {
    findOrFetchAndSave: async (tmdbid) => {
      try {
        ...
        let moviemeta = await strapi.db.query('api::moviemeta.moviemeta').findOne({
          where : {tmdbid: tmdbid}
        });
        if (!moviemeta) {

          const response = await axios.get(`${apiBaseUrl}/movie/${tmdbid}?${qs.stringify(defaultOptions)}`);
          ...
          let colors = {};
          if (poster) {
             colors = await strapi.service('api::shared.shared').getImageSwatches(`${posterBaseUrl}${poster}`);
          }
