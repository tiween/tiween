...
module.exports = {
  async beforeCreate(event) {
    ...
    if (tmdbid) {
      ...
      const moviemeta = await strapi.service('api::moviemeta.moviemeta').findOrFetchAndSave(tmdbid);
      
      event.params.data.day = DateTime.fromISO(event.params.data.date).toFormat('yyyy-MM-dd');
      ...
      event.params.data.runtime = get(event, ['params','data','creative_work', 'runtime'], 0);
    }
  },
  ...
  async afterCreate(showtime) {
    ...
    if (showtime.recurring && !isEmpty(showtime.recurringRule)) {
      ...
      const rrule = rrulestr(showtime.recurringRule, { dtstart: showtime.date, tzid: 'Africa/Tunis' });
      ...
      if (occurences.length > 0) {
        const promises = occurences.map(date => {
          ...
          return strapi.service('api::showtime.showtime').create(showtimeOccurence);
        });
