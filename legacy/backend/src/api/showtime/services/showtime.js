...
module.exports = createCoreService('api::showtime.showtime', ({ strapi }) => {
    return {
        ...
        async create(data) {
            const validData = await strapi.entityValidator.validateEntityCreation(strapi.getModel('api::showtime.showtime'), data);
            const { tmdbid, date } = validData.data;

            const moviemeta = await strapi.service("api::moviemeta.moviemeta").findOrFetchAndSave(validData.data.tmdbid);
            ...
            const entry = await strapi.query('api::showtime.showtime').create(showtimeToInsert);
            console.log("-----------------------------------", entry);
            return entry;
        },
        async movie(params) {
            ...
            const showtimes = await strapi.db.query('api::showtime.showtime').findMany({
                where: {
                    date_gt: moment().toISOString(),
                    tmdbid: tmdbid
                }
            });
            const groupedShowtimes = reduce(showtimes, (result, value) => {
                const day = value.day;
