...
module.exports = {
  ...
  async mediumsAutoComplete(ctx) {
    try {
      ...
      const mediums = await strapi.entityService.findMany(
        "api::medium.medium",
        params
      )
  ...
  async eventGroupsAutoComplete(ctx) {
    try {
      const response = await strapi
        .service("api::event-group.event-group")
        .find(ctx.query)
  ...
  async createEvent(ctx) {
    ...
    const validData = await strapi.entityValidator.validateEntityCreation(
      strapi.getModel("api::event.event"),
      {
        medium,
        event_group,
        startDate,
    ...
    if (showtimes.length > 0) {
      createdShowtimes = await Promise.all(
      ...
      let runtime = 0
      const title = createdShowtimes
        .map((item) => {
          console.log("item", item)
          if (item.moviemeta) {
            runtime += parseInt(
              get(item, ["moviemeta", "remote", "runtime"], 10)
            )
      ...
      if (firstShowtime.moviemeta) {
        ...
        colors = get(
          firstShowtime,
          ["creative_work", "poster", "colors"],
          get(firstShowtime, ["creative_work", "photos", 0, "colors"], {})
        )
      }
      // Update event with runtime
      createdEvent = await strapi.entityService.update(
        "api::event.event",
        createdEvent.id,
        {
          data: {
      ...
      if (createdEvent.recurring && !isEmpty(createdEvent.recurringRule)) {
        ...
        if (occurences.length > 0) {
          ...
          for (let i = 0; i < occurences.length; i++) {
            ...
            const createdRepeatingEvent = await strapi.entityService.create(
              "api::event.event",
              {
                data: { repeatingEvent },
                populate: "showtimes",
            ...
            if (showtimes.length > 0) {
              const createdShowtimes = await Promise.all(
                showtimes.map((showtime) =>
                  strapi.service("api::showtime.showtime").create({
                    data: {
                      ...showtime,
                      date: eventDate,
              ...
              for (const item of createdShowtimes) {
                if (item.moviemeta) {
                  title.push(
                    `${item.moviemeta.remote.title} [${item.language}][${item.video_format}]`
                  )
                  runtime += parseInt(
                    get(item, ["moviemeta", "remote", "runtime"], 10)
                  )
                  ...
                  const type =
                    item.creative_work.type === "SHORT_MOVIE" ? "CM" : "TH"
                  title.push(
                    `${item.creative_work.title} [${item.language}][${type}]`
                  )
                  runtime += parseInt(
                    get(item, ["creative_work", "runtime"], 0),
                    10
                  )
                }
              }
  ...
  async createEventShowtimes(showtimes, event) {
    ...
    for (const item of showtimes) {
      try {
        if (item.moviemeta) {
          title.push(
            `${item.moviemeta.title} [${item.language}][${item.video_format}]`
          )
