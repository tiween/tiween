...
import { BiTrash as DeleteIcon, BiVolumeLow as LanguageIcon } from 'react-icons/bi';
...
const EventsCalendar = (props) => {
  ...
  const [state, dispatch] = useReducer(reducer, { displayWarning: false, displayShowtimeSettings: false });
    ...
    return (
      <StyledCalendarEvent>
        <div className="d-flex justify-content-between">
          <i>{info.timeText}</i>
          <button className="btn btn-link p-0 delete-showtime" style={{ color: 'white', textDecoration: 'none' }} onClick={(event) => {

            event.stopPropagation();
            dispatch({ type: 'delete.confirm', payload: { id: info.event.id } });
            setIsVisible(true);
            return false;
        ...
        <ul className="list-unstyled pl-1" style={{ position: 'relative', bottom: 0 }}>
          ...
          {eventGroup ? (<li className="text-uppercase">
            <span>
              {eventGroup.shortTitle}
            </span>
          </li>) : (<></>)
          }
  ...
  const visibleRangeEndDate = DateTime.fromISO(currentDate).plus({ months: 4 }).toFormat('yyyy-MM-dd');
          ...
          <DialogBody icon={<ExclamationMarkCircle />}>
            <Stack spacing={2}>
              <Flex justifyContent="center">
                <Typography id="confirm-description">Voulez vous supprimer cet évenement?</Typography>
              </Flex>
            </Stack>
          </DialogBody>
          <DialogFooter startAction={<Button onClick={() => { setIsVisible(false); console.log(state.displayWarning) }} variant="tertiary">
            Annuler
          </Button>} endAction={<Button variant="danger-light" startIcon={<Trash />} onClick={async () => {
            if (state.eventToDelete) {
              await request(`/events-manager/events/${state.eventToDelete}`, { method: 'DELETE' })
              setIsVisible(false)
            }
          }}>
            Confirmer
