...
const noPoster = 'https://res.cloudinary.com/tiween/image/upload/c_scale,q_auto,w_50/v1573680548/film-poster-placeholder.png';
const size = 'w45';
...
function TMDBMovieSelectorOption({ innerProps, isDisabled, data, ...rest }) {
  ...
  const posterFullPath = isEmpty(posterPath) ? `${noPoster}` : `${posterBaseUrl}/${size}${posterPath}`;
        ...
        <Flex gap={1} alignItems="start">
          ...
          <Stack spacing={2}>
            <Stack horizontal spacing={1}>
              ...
              <Typography variant="pi" fontWeight="bold">{data.title}</Typography>
            </Stack>
            {data.original_title && data.title !==  data.original_title&& <Typography variant="pi">{data.original_title}</Typography>}
          </Stack>
