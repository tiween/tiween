...
const EventSettings = ({ start, isOpen, onClosed, onToggle, medium, eventGroup }) => {
  ...
  useEffect(() => {
    const fetchEventContentType = async () => {
      ...
      try {
        const { data } = await request('/content-manager/content-types/api::event.event/configuration');
    ...
    const getShowtimeContentSchema = async () => {
      ...
      try {
        const { data } = await request('/content-type-builder/content-types/api::showtime.showtime');
  ...
  const initialFormValues = useMemo(() => {
    ...
    const fields = map(metadatas, (attribute, index) => {
      const defaultValue = get(eventContentType, ['schema', 'attributes', index, 'default'], '');
      ...
      .filter((attribute) => {
        return attribute.editable && attribute.fieldName !== 'createdAt' && attribute.fieldName !== 'updatedAt';
      });
        ...
        <ModalLayout onClose={onClosed} labelledBy="title">
          ...
          <Formik
            initialValues={{
              ...
              showtimes: [{
                ...
                language: get(showtimeContentSchema, ['attributes', 'language', 'default']),
                video_format: get(showtimeContentSchema, ['attributes', 'video_format', 'default']),
                premiere: get(showtimeContentSchema, ['attributes', 'premiere', 'default'], false),
              }],
            }}
            enableReinitialize
          ...
          >
            {({ values, errors, dirty, isValid }) => {
              ...
              return (
                <Form>
                  {errors?.length > 0 && <Alert closeLabel="Fermer" title="Fonctionnalité desactivée" onClose={() => {

                  }}>
                    Cette fonctionnalité a été desactivée intentionnelemment en attendant de régler quelques bugs.
                      ...
                      >
                        <div className="shadow-sm bg-white rounded py-3 px-4">
                          <FieldArray
                            ...
                            render={arrayHelpers => (
                              <Stack spacing={2} >
                                {values.showtimes && values.showtimes.length > 0 && (
                                ...
                                <ShowtimeTypeButtons handleOnClick={(eventType) =>
                                  arrayHelpers.push({
                                    type: eventType,
                                    language: get(showtimeContentSchema, ['attributes', 'language', 'default']),
                                    video_format: get(showtimeContentSchema, ['attributes', 'video_format', 'default']),
                                    premiere: get(showtimeContentSchema, ['attributes', 'premiere', 'default']),
                                  })
                                }
                                />
                              </Stack>
                          ...
                          {showAlert && (
                            <Stack spacing={1}>
                              ...
                              <Alert closeLabel="Close alert" title="Fonctionnalité desactivée" onClose={() => {
                                setShowAlert(false)
                              }}>
                                Cette fonctionnalité a été desactivée intentionnelemment en attendant de régler quelques bugs.
                                Navré pour la gène occasionnée.
