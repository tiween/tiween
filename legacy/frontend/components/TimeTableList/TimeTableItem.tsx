...
const TimetableItem: React.FC<TimetableItemProps> = ({ show, withMedium = false, runtime = 0 }) => {
  return (
    <div
    ...
    >
      <div className="flex flex-col w-full  text-base font-medium  justify-around items-center text-white font-fira">
        <div className="time text-2xl flex flex-col leading-5">
          <span>
            {DateTime.fromISO(show.event.fullStartDate, { zone: 'Africa/Tunis' }).toFormat(
              "HH'h'mm",
            )}
          </span>
          ...
          {runtime > 0 && (
            <span className="font-thin  text-xs mt text-center">
              (fin:&nbsp;
              {DateTime.fromISO(show.event.fullStartDate, { zone: 'Africa/Tunis' })
