...
const SingleWork: React.FunctionComponent<SingleWorkProps> = ({ event, children, rightBlock }) => {
  ...
  return (
    <div className="event-block-wrapper relative flex  flex-col-reverse md:flex-row justify-start items-start bg-bastille rounded-sm px-3 py-4">
      ...
      <div className="event-showtimes flex-grow md:mb-0 mb-3">{<WorkBlock show={show} />}</div>
      <div>{rightBlock}</div>
    </div>
  );
};
