...
const AgendaForm: React.FunctionComponent<{ handleSubmit: (values: Values) => void }> = ({
  handleSubmit,
}) => {
  return (
        ...
        {({ values, setFieldValue, getFieldProps, isSubmitting, errors }) => {
          return (
            <Form className="flex flex-col space-y-2">
              <label htmlFor="email" className="block text-md  text-selago font-semibold">
                Créez un Agenda
              </label>
              <div
              ...
              >
                <div className="relative flex items-stretch flex-grow focus-within:z-10">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <CalendarIcon className="h-5 w-5 text-gray-700" aria-hidden="true" />
                  </div>
                  <input
                    type="text"
                    {...getFieldProps('name')}
              ...
              {errors['name'] && (
                <span className="text-amaranth text-xs font-semibold">{errors['name']}</span>
              )}
              <Switch.Group as="div" className="flex items-start justify-between">
                <span className="flex-grow flex flex-col justify-between">
                  <Switch.Label as="span" className="text-sm font-semibold text-gray-300" passive>
                    Public
                  </Switch.Label>
                  <Switch.Description as="span" className="text-xs text-gray-400">
                    Votre agenda est privé, rendez-le public pour le partager avec vos amis !
                  </Switch.Description>
                </span>
                <Switch
                ...
                >
                  ...
                  >
                    <span
                      ...
                      aria-hidden="true"
                    >
                      <svg className="h-2 w-2 text-gray-600" fill="none" viewBox="0 0 12 12">
                        <path
