...
const handler = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  ...
  const selectedDay = parseInt(
    DateTime.fromISO(query['date'] as string, { zone: 'Africa/Tunis' }).toFormat('o'),
    10,
  );

  let startDate;
  if (selectedDay === ordinalToday) {
    startDate = DateTime.fromISO(query['date'] as string, { zone: 'Africa/Tunis' })
      .set({
        hour: today.hour,
        minute: today.minute,
      })
  ...
  const endDate = DateTime.fromISO(query['date'] as string, { zone: 'Africa/Tunis' })
    .endOf('day')
    .toISO();
  const variables = {
