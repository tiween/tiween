...
const extractMovieStructedData = (movie: TMDBMovie): Object => {
  ...
  if (directors.length > 1) {
    ...
    set(movieStructredData, 'director', { '@type': 'Person', name: directors[0] });
  }
...
export const movieShowtimesStructredData = (movie: TMDBMovie, timetable: unknown): string => {
  ...
  const screeningEventsWithPosition = flatten(screeningEvents).map((item, index) => {
    return {
      '@type': 'ListItem',
