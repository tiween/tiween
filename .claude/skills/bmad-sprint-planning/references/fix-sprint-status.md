...
Rebuild `sprint-status.yaml` to a pristine, script-valid state when it is broken, hand-mangled, drifted from reality, or the user simply asks to fix it. Inference determines what the state _should_ be; the user confirms it; the script writes it. Never write without the confirmation.
