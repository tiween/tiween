...
if (unattributed.length > 0) {
  ...
  throw new Error(
    `${unattributed.length} violation(s) carry no registry row; re-run these workers: ${dimensions.join(", ")}`
  )
}
...
const criticalSeverity = dedupedViolations.filter(
...
```javascript
const deductions =
  violationSummary.CRITICAL * 10 +
  violationSummary.HIGH * 5 +
  violationSummary.MEDIUM * 2 +
  violationSummary.LOW * 1
```
