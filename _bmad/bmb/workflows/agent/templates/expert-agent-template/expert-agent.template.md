...
menu:
{{#each menu_items}} - trigger: {{trigger_code}} or fuzzy match on {{trigger_command}}
{{#if action_is_prompt}}
action: '#{{action_id}}'
{{else}}
...
questions:
{{#each install_questions}} - var: {{var_name}}
prompt: '{{prompt}}'
type: {{question_type}}{{#if question_options}}
options:
{{#each question_options}} - label: '{{label}}'
value: '{{value}}'
{{/each}}
{{/if}}
